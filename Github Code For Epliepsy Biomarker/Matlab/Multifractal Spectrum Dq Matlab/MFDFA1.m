...
if Fig==1, 
   ...
   %---------------------------
    ...
    % ---- FIX: enforce palette & consistency for Panel 1.1 ----
    % Colors (min(q)=red, 0=blue, max(q)=green)
    col_min = [1 0 0];
    col_mid = [0 0.4470 0.7410];
    col_max = [0 0.498 0];

    % Dots (make face = edge = line color; keep circles)
    set(plot1(1), 'Marker','o','LineStyle','none', ...
        'MarkerFaceColor',col_min, 'MarkerEdgeColor',col_min, ...
        'Color',col_min, 'DisplayName',strcat('q = ',num2str(min(q))));
    set(plot1(2), 'Marker','o','LineStyle','none', ...
        'MarkerFaceColor',col_mid, 'MarkerEdgeColor',col_mid, ...
        'Color',col_mid, 'DisplayName',strcat('q = ',num2str(q_middle)));
    set(plot1(3), 'Marker','o','LineStyle','none', ...
        'MarkerFaceColor',col_max, 'MarkerEdgeColor',col_max, ...
        'Color',col_max, 'DisplayName',strcat('q = ',num2str(max(q))));

    % Regression lines (pure lines; no markers; matching colors)
    set(plot1(4),'LineWidth',2,'LineStyle','-','Marker','none', ...
        'Color',col_min,'DisplayName',strcat('q = ',num2str(min(q))));
    set(plot1(5),'LineWidth',2,'LineStyle','-','Marker','none', ...
        'Color',col_mid,'DisplayName',strcat('q = ',num2str(q_middle)));
    set(plot1(6),'LineWidth',2,'LineStyle','-','Marker','none', ...
        'Color',col_max,'DisplayName',strcat('q = ',num2str(max(q))));

    % Rebuild legend from the colored DOT handles so chips are circles
    legend(subplot1, [plot1(1) plot1(2) plot1(3)], ...
        {['q = ' num2str(min(q))], ['q = ' num2str(q_middle)], ['q = ' num2str(max(q))]}, ...
        'Location','southeast','AutoUpdate','off');
    % ---- END FIX for Panel 1.1 ----
