...
OUTPUT_DIR_NAME = "results_02_adult_random_forest"
N_SPLITS = 10
RANDOM_STATE = 42
THRESHOLD = 0.50
...
MANUAL_TEST_FOLDS = [
    ...
    ["DETROIT017", "UCLA25"],
    ["DETROIT083", "PY19N026"],
    ["DETROIT006", "NIH4", "UCLA26"],
    ["DETROIT039", "NIH1"],
    ["DETROIT035", "PY18N015"],
    ["DETROIT071", "PY19N023"],
    ["DETROIT093", "NIH3", "UCLA03"],
    ["DETROIT032", "NIH5", "UCLA01"],
]
...
def create_splits(data: pd.DataFrame):
    ...
    available = set(data[ID_COLUMN].unique())
    specified = [patient for fold in MANUAL_TEST_FOLDS for patient in fold]
    if len(specified) != len(set(specified)):
        raise ValueError("A patient appears in more than one manual adult test fold.")
    if set(specified) != available:
        missing = sorted(available - set(specified))
        extra = sorted(set(specified) - available)
        raise ValueError(f"Manual folds do not match the CSV. Missing={missing}; Extra={extra}")
    for test_patients in MANUAL_TEST_FOLDS:
