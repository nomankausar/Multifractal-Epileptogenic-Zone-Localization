...
# ========== Load Dataset ==========
file_path = r"F:\Research After Conference\task 77 ml on child 5min  cleaned\ml_child.xlsx"
...
for name, model in models.items():
    ...
    plt.show()
    # ROC Curve (mean + shaded std across folds)
    if len(tprs) == 0:
        print(f" ROC skipped for {name}: no valid folds collected.")
    else:
        plt.figure(figsize=(8, 6))
