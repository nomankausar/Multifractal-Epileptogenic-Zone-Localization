...
# -*- coding: utf-8 -*-
"""
Pediatric Random Forest — Patient-Grouped 10-Fold CV
...
Spyder use
----------
1. Place this script beside pediatric_features.xlsx or .csv.
2. Open the script in Spyder.
3. Press Run.
4. Results are saved in outputs/pediatric_random_forest.
...
COHORT = "pediatric"
DATA_BASENAME = "pediatric_features"
...
OUTPUT_DIR = Path(
    os.environ.get(
        "EZ_OUTPUT_DIR",
        str(SCRIPT_DIR / "outputs" / "pediatric_random_forest"),
    )
)
...
def build_model(y_train: np.ndarray, fold_seed: int):
    estimator = RandomForestClassifier(
        n_estimators=50 if QUICK_TEST else 500,
        max_depth=None,
        min_samples_leaf=5,
        max_features="sqrt",
        class_weight="balanced_subsample",
